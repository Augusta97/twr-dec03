    ...
    <b-row>
      <b-col sm="6">
        <b-row v-for="(mem, inx) in userAtten" :key="inx">
          ...
          <b-col sm="6">
            <p :class="mem.attendanceStatus == 'A' ? 'clr-trash' : ''">
...
export default {
  ...
  computed: {
    ...
    uniqueYr: function () {
      return [
        ...new Set(this.userAtten.map((x) => x.attendanceDate.substr(6, 2))),
      ];
    },
  },
  mounted() {
    for (var x = 0; x < this.grpAttendance.length; x++) {
      for (
        var k = 0;
        k <
        this.grpAttendance[x].attendance.filter(
          (t) => parseInt(t.userId) === parseInt(this.$route.params.id)
        ).length;
        k++
      ) {
        this.userAtten.push({
          attendanceDate: this.grpAttendance[x].attendanceDate,
          attendanceStatus: this.grpAttendance[x].attendance[k]
            .attendanceStatus,
        });
      }
    }
