<template>
  <form class="marginTop30">
    <b-row>
      <b-col sm="7" class="d-flex">
        <p class="col-sm-6 text-left">
          <b>Group name</b>
          <span class="spacelr">{{ groupList[groupId - 1].groupName }}</span>
        </p>
        <p class="col-sm-6 text-right">
          <b class="spacelr">Date</b> <span>{{ attDate }}</span>
        </p>
      </b-col>
    </b-row>
    <b-row>
    ...
    <b-row
      v-for="mem in grpAttendance.filter(
        (x) => x.groupId == groupId && x.attendanceDate == attDate
      )"
    ...
    >
      <b-col
        sm="7"
        class="d-flex"
        v-for="(filterAttendance, inx) in mem.attendance"
        :key="inx"
      >
        <p class="col">
