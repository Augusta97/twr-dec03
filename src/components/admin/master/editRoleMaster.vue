<template>
  <div class="scroll-y">
    <div v-for="(role, index) in roles" :key="index" class="marTop10">
      <div v-if="role.status == 0" class="d-flex">
        <div v-if="role.roleId != roleClick" class="col-sm-1 col-md-2">
          {{ role.roleType }}
        </div>
        <div v-else class="col-sm-4">
          <form @submit.prevent="updateRole()">
            ...
            <span v-show="errors.has('editroleType')" class="help is-danger">
              {{ errors.first("editroleType") }}
            </span>
          </form>
        </div>
        <div v-if="role.roleId != roleClick" class="spacelr">
          <font-awesome-icon
            v-b-tooltip.hover
            v-b-modal.modaledit
            icon="edit"
