<template>
  <div class="scroll-y">
    <div v-for="(country, index) in countryList" :key="index" class="marTop10">
      <div v-if="country.status == 0" class="d-flex">
        <div v-if="country.countryId != countryClick" class="col-sm-1 col-md-2">
          {{ country.countryName }}
        </div>
        <div v-else class="col-sm-4">
          <form @submit.prevent="updateCountry">
            ...
            <span v-show="errors.has('editcountryName')" class="help is-danger">
              {{ errors.first("editcountryName") }}
            </span>
          </form>
        </div>
        <div v-if="country.countryId != countryClick" class="spacelr">
          <font-awesome-icon
            v-b-tooltip.hover
            v-b-modal.modaledit
            icon="edit"
