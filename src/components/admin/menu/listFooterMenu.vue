            ...
            <b-col md="10" class="mb-2">
              ...
              >
                <template v-slot:cell(action)="row">
                  <i
                    @click="viewFootMenu(row.item.ID)"
                  ...
                  ></i>
                  <i
                    @click="editFootMenu(row.item.ID)"
                    v-b-modal.modal-xl
                    v-b-tooltip.hover
                    title="edit"
              ...
              <div class="viewtable-width">
                <b-col md="6" class="my-1">
                  <b-pagination
                    v-model="currentPage"
                    :total-rows="allFooterMenu.length"
                    :per-page="perPage"
                    class="my-0"
                  ></b-pagination>
                </b-col>
  ...
  methods: {
    ...
    editFootMenu(ID) {
      if (localStorage.getItem("userId") == null) {
        ...
        this.$router.push({ path: "/EditFooterMenu/" + ID });
      }
    },
    viewFootMenu(ID) {
      if (localStorage.getItem("userId") == null) {
        ...
        this.$router.push({ path: "/EditFooterMenu/" + ID + "/view" });
      }
    },
  },
  computed: {
    ...mapState(["allFooterMenu"]),
  },
};
</script>
