          ...
          >
            <b-row>
              <b-form-group class="col-sm-12">
                <h5 v-if="$route.params.view === 'view'">View Event</h5>
            ...
            <b-row v-if="$route.params.view === 'view'">
              <b-col sm="9" class="marginTop">
                ...
                <b-form-group class="col-sm-12">
                  <label for="excerpt">Location</label>
                  <b-form-textarea
                    type="text"
                    name="excerpt"
                ...
                <b-row>
                  <b-col>
                    <b-form-group class="col-sm-12">
                      ...
                      <datepicker
                        ...
                        :class="{
                          input: true,
                          'is-danger': errors.has('startDate'),
                        }"
                        readonly
                      ></datepicker>
                      <span
                  ...
                  <b-col>
                    <b-form-group class="col-sm-12">
                      <label for="registerDate">End Date</label>
                      <datepicker
                        placeholder="Select Date"
                  ...
                  <b-col>
                    <b-form-group class="col-sm-12">
                      ...
                      <datepicker
                        ...
                        v-validate="'required'"
                        :format="format"
                        v-on:selected="changeDisableDates"
                        :class="{
                          input: true,
                          'is-danger': errors.has('startDate'),
                        }"
...
export default {
  ...
  computed: {
    ...mapState(["existinglang", "allEvents"]),
    commonIndex() {
      if (this.allEvents.row !== null && this.allEvents.row.length !== 0) {
        return this.allEvents.row.findIndex(
  ...
  methods: {
    titleCheck() {
      let pa = this.allEvents.row.findIndex(
    ...
    changeDisableDates() {
      var self = this;
      this.$nextTick(() => this.changeToDisableDate(self.formData.startDate));
    },
    changeToDisableDate(newDateChange) {
      this.disabledDates.to = newDateChange;
      let a = new Date(this.formData.endDate);
      let end = a.getMonth() + 1 + "-" + a.getDate() + "-" + a.getFullYear();
      let b = new Date(newDateChange);
      let start = b.getMonth() + 1 + "-" + b.getDate() + "-" + b.getFullYear();
      if (start > end) {
        this.formData.endDate = "";
      }
    },
    ...mapActions(["editEventById"]),
        ...
        this.$validator.validateAll().then((valid) => {
          if (valid && !this.titleExists) {
            const payload = {
              ...
              ID: this.allEvents.row[this.commonIndex].ID,
              ...
              bannerImageUrl:
                this.formData.bannerImageUrl === ""
                  ? this.allEvents.row[this.commonIndex].bannerImageUrl
                  : this.formData.bannerImageUrl,
  ...
  created() {
    this.formData.title = this.allEvents.row[this.commonIndex].title;
    this.formData.excerpt = this.allEvents.row[this.commonIndex].excerpt;
    this.formData.status = this.allEvents.row[this.commonIndex].status;
    this.formData.expire = this.allEvents.row[this.commonIndex].expire;
    this.formData.language = this.allEvents.row[this.commonIndex].language;
    this.formData.bannerImageUrl =
      this.allEvents.row[this.commonIndex].bannerImageUrl !== ""
        ? this.allEvents.row[this.commonIndex].bannerImageUrl
        : "";
    this.formData.startDate = this.allEvents.row[this.commonIndex].startDate;
    this.formData.endDate = this.allEvents.row[this.commonIndex].endDate;
    this.formData.latitude = this.allEvents.row[this.commonIndex].latitude;
    this.formData.longitude = this.allEvents.row[this.commonIndex].longitude;
    this.formData.location = this.allEvents.row[this.commonIndex].location;
  },
  mounted() {
    this.formData.content = new Editor({
      ...
      content: this.allEvents.row[this.commonIndex].content,
    });
  },
  beforeDestroy() {
