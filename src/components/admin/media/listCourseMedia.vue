    ...
    <b-row class="form-height">
      ...
      <b-col md="10" class="mb-2 form-bg">
        ...
        <b-container fluid class="bg-white">
          <b-row class="form-height">
            <b-col md="12" class="mb-2 pt-2">
              <b-col class="phasetable-space d-flex pb-2">
              ...
              >
                ...
                <template v-slot:cell(action)="row">
                  <div class="action-cls">
                    <i
                      @click="viewMedia(row.item.ID)"
                      class="fa fa-eye action-icons eye-active"
                      style="width: 20%; color: rgb(6 51 125)"
                      v-b-tooltip.hover
                      title="View"
                    ></i>
                    <i
                      @click="editMedia(row.item.ID)"
                      v-b-tooltip.hover
                      title="edit"
              ...
              <div class="viewtable-width">
                <b-col md="6" class="my-1">
                  <b-pagination
                    v-model="currentPage"
                    :total-rows="filterCourseMedia.length"
                    :per-page="perPage"
                    class="my-0"
                  ></b-pagination>
                </b-col>
  ...
  methods: {
    ...
    editMedia(ID) {
      if (localStorage.getItem("userId") == null) {
        ...
        this.$router.push({ path: "/EditMedia/" + ID });
      }
    },
    viewMedia(ID) {
      if (localStorage.getItem("userId") == null) {
        ...
        this.$router.push({ path: "/EditMedia/" + ID + "/view" });
      }
    },
  },
  computed: {
    ...
    filterCourseMedia() {
      if (this.allMedia.row !== null && this.allMedia.row.length !== 0) {
        return this.allMedia.row.filter((x) => x.mediaClassification == 2);
      } else {
        return -1;
      }
    },
