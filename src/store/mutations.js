export default {
  ...
  appendLangType: (state, { data }) => {
    state.existinglang.push(data);
  },
  appendPlans: (state, { plan }) => {
    state.plan.push(plan);
  },
  appendEnglishLabel: (state, { data }) => {
    state.LabelList.push(data);
  },
  appendVersion: (state, { data }) => {
    state.versionByLanguage.push(data);
  },
  appendPlanType: (state, { data }) => {
    state.planType.push(data);
  },
  appendGroup: (state, { data }) => {
    state.groupList.push(data);
  },
  assignEdit: (state, { assignupdate, data }) => {
    state.assignedMembers[assignupdate - 1].status = data.status;
  },
  assignroleAndgrp: (state, { data, index }) => {
  ...
  appendGroupAttendance: (state, { data }) => {
    ...
    for (var t = 0; t < state.grpAttendance.length; t++) {
      ...
      state.grpAttendance[t].attendance.sort((a, b) =>
        a.firstname.localeCompare(b.firstname)
      );
    }
    state.grpAttendance = state.grpAttendance
      .slice()
      .sort((a, b) => b.attendanceDate - a.attendanceDate);
    for (var k = 0; k < state.grpAttendance.length; k++) {
      ...
      var newOrder =
        (reDate <= 9 ? "0" + reDate : reDate) +
        "-" +
        (reMonth <= 9 ? "0" + reMonth : reMonth) +
        "-" +
        reYr.substr(2);
    ...
    for (var q = 0; q < state.grpAttendance.length; q++) {
      for (var w = 0; w < uniqueArray(arr).length; w++) {
        state.grpAttendance[q].attendance.findIndex(
          (x) =>
            (x.userId === uniqueArray(arr)[w].userId) === -1 &&
            x.groupId === uniqueArray(arr)[w].groupId
        ) !== -1
          ? state.grpAttendance[q].attendance.push({
  ...
  updateRole: (state, { roleType, index }) => {
    state.roles[index].roleType = roleType;
  },
  updateCountry: (state, { countryName, index }) => {
    state.countryList[index].countryName = countryName;
  },
  updatePriority: (state, { priorityName, index }) => {
    state.priorityList[index].priorityName = priorityName;
  },
  deleteRoleStatus: (state, { inx, data }) => {
    state.roles[inx].data = data;
  },
  deleteCountryStatus: (state, { inx, data }) => {
    state.countryList[inx].data = data;
  },
  deletePriorityStatus: (state, { inx, data }) => {
    state.priorityList[inx].data = data;
  },
  deleteLangStatus: (state, { index, status }) => {
    state.existinglang[index].status = status;
  },
  deleteEngStatus: (state, { index, status }) => {
    state.LabelList[index].status = status;
  },
  deleteAllLabelStatus: (state, { index, status }) => {
    state.AllLabel[index].status = status;
  },
  deleteGrpStatus: (state, { index, data }) => {
    state.groupList[index].data = data;
  },
  deleteUserStatus: (state, { index, data }) => {
    var delIndex = state.assignedMembers.findIndex(
      (y) => y.assignId === data.assignId
    );
    state.users[index].roleId = data.roleId;
    state.users[index].roleType = data.roleType;
  ...
  deleteLeaderStatus: (state, { data }) => {
    var delIndex = state.assignedMembers.findIndex(
      (y) => y.assignId === data.assignId
    );
    state.assignedMembers[delIndex].status = data.status;
  },
  transferMembers: (state, { data }) => {
    var transIndex = state.assignedMembers.findIndex(
      (y) => y.assignId === data.assignId
    );
  ...
  updatePassword: (state, { email, newpassword }) => {
    state.email = email;
    state.password = newpassword;
  },
  updateAttendance: (state, { data }) => {
    var m = state.grpAttendance.findIndex(
      (y) =>
        parseInt(y.groupId) === parseInt(data.groupId) &&
        y.attendanceDate === data.attendanceDate
    );
    var Indx = state.grpAttendance[m].attendance.findIndex(
      (y) => y.attendanceId === data.attendanceId
    );
    state.grpAttendance[m].attendance[Indx].attendanceStatus =
      data.attendanceStatus;
  },
  appendDenomination: (state, { data }) => {
    state.denominationList.push(data);
  },
  updateDenomination: (state, { denominationName, index }) => {
    state.denominationList[index].denominationName = denominationName;
  },
  deleteDenominationStatus: (state, { inx, data }) => {
  ...
  updateChurch: (state, { data, index }) => {
    state.churchList[index] = data;
  },
  deleteChurchStatus: (state, { inx, data }) => {
    state.churchList[inx].data = data;
  },
  updateGroup: (state, { data, index }) => {
  ...
  updateMedia: (state, { data }) => {
    ...
    state.generalMedia[data.index].mediaClassification =
      data.mediaClassification;
  ...
  deletePersonalInfoStatus: (state, { index, data, rolname }) => {
    if (rolname === "usr") {
      state.users[index] = data;
    } else {
      state.superusers[index] = data;
    }
  },
  appendBible: (state, { data }) => {
    state.bibleType.push(data);
  },
  loadAllBible: (state, bibleType) => {
    state.bibleType = bibleType;
  },
  updateBible: (state, { data }) => {
  ...
  // },
  deleteUpload: (state, { inx, data }) => {
    state.groupUploads[inx].status = data.status;
  },
  appendHelp: (state, { data }) => {
    // x.slice().sort((a, b) => new Date(a.attendanceDate) - new Date(b.attendanceDate))
  ...
  appendHelpAnswer: (state, { index, answer }) => {
    // x.slice().sort((a, b) => new Date(a.attendanceDate) - new Date(b.attendanceDate))
